...
from lib.gnc import Hmtrx, crossFlowDrag, forceLiftDrag, gvect, m2c, ssa
...
class remus100:
    ...
    def __init__(
        ...
        self.ref_z = r_z  # reference depth
        self.ref_psi = r_psi * self.D2R  # reference heading
        self.ref_n = r_rpm  # reference propeller revolution
        self.V_c = V_current  # current speed
        self.beta_c = beta_current * self.D2R  # current direction
        self.controlMode = controlSystem  # control mode
        ...
        self.nu = np.array([0, 0, 0, 0, 0, 0], float)  # velocity vector (6,)
        self.u_actual = np.array([0, 0, 0], float)  # control input vector (3,)


        self.controls = [
            "Tail rudder (deg)",  # 尾舵
            "Stern plane (deg)",  # 舵板
            "Propeller revolution (rpm)",  # 螺旋桨转速
        ]
        self.dimU = len(self.controls)  # 控制输入维度

        # Actuator dynamics
        self.deltaMax_r = 15 * self.D2R  # max rudder angle (rad)
        self.deltaMax_s = 15 * self.D2R  # max stern plane angle (rad)
        self.nMax = 1525  # max propeller revolution (rpm)  with max speed of 2.5m/s
        self.T_delta = 0.1  # rudder/stern plane time constant (s)
        self.T_n = 1.0  # propeller time constant (s)

        # Raise exception about propeller revolution
        if r_rpm < 0.0 or r_rpm > self.nMax:
            sys.exit("The RPM value should be in the interval 0-%s", (self.nMax))

        # Raise exception about desired depth
        ...
        # NOTE calculate added mass matrix by ＊＊estimate＊＊ the fluid dynamics coefficients
        ...
        # Feed forward gains (Nomoto gain parameters) 前馈增益(Nomoto增益参数)
        self.K_nomoto = 5.0 / 20.0  # K_nomoto = r_max / delta_max
        self.T_nomoto = self.T_yaw  # Time constant in yaw

        # Heading autopilot reference model 航向自动驾驶参考模型
        ...
        # Heading autopilot (Equation 16.479 in Fossen 2021) 航向自动驾驶
    ...
    def dynamics(self, eta, nu, u_actual, u_control, sampleTime):
        ...
        delta_r_c = u_control[0]  # commanded tail rudder (rad) 尾舵命令
        delta_s_c = u_control[1]  # commanded stern plane (rad) 舵板命令
        n_c = u_control[2]  # commanded propeller revolution (rpm) 螺旋桨转速命令

        delta_r = u_actual[0]  # actual tail rudder (rad) 尾舵实际
        delta_s = u_actual[1]  # actual stern plane (rad) 舵板实际
        n = u_actual[2]  # actual propeller revolution (rpm) 螺旋桨转速实际

        # Amplitude saturation of the control signals
        # 限制输出，如果超过上下限，就让他等于上下限
        ...
        # NOTE Rigi-body/added mass Coriolis/centripetal matrices expressed in the CO
        ...
        C = CRB + CA  # Coriolis/centripetal矩阵
    ...
    def depthHeadingAutopilot(self, eta, nu, sampleTime):
        ...
        delta_s = (
            -self.Kp_theta * ssa(theta - theta_d)
            - self.Kd_theta * q
    ...
    def forward(self,eta,nu,tau,sampleTime):
        u_c = self.V_c * math.cos(self.beta_c - eta[5])  # current surge velocity
        v_c = self.V_c * math.sin(self.beta_c - eta[5])  # current sway velocity
