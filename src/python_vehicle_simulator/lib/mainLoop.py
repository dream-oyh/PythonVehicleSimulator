...
def printSimInfo():
    ...
    print(
        "1 - Deep submergence rescue vehicle (DSRV): controlled by a stern plane, L = 5.0 m"
    )
    print(
        "2 - Frigate: rudder-controlled ship described by a nonlinear Nomoto model, L = 100.0 m"
    )
    print(
        "3 - Otter unmanned surface vehicle (USV): controlled by two propellers, L = 2.0 m"
    )
    print(
        "4 - ROV Zefakkel: rudder-controlled ship described by a nonlinear Nomoto model, L = 54.0 m"
    )
    print(
        "5 - Semisubmersible: controlled by tunnel thrusters and main propellers, L = 84.5 m"
    )
    print(
        "6 - Ship: linear maneuvering model specified by L, B and T using the Clarke (1983) formulas"
    )
    print(
        "7 - Offshore supply vessel: controlled by tunnel thrusters and main propellers, L = 76.2 m"
    )
    print(
        "8 - Tanker: rudder-controlled ship model including shallow water effects, L = 304.8 m"
    )
    print(
        "9 - Remus 100: AUV controlled by stern planes, a tail rudder and a propeller, L = 1.6 m"
    )
    print(
        "10 - Torpedo: AUV controlled by configurable fins and a propeller, L = 1.6 m"
    )
    print(
        "---------------------------------------------------------------------------------------"
    )
...
###############################################################################
def simulate(N, sampleTime, vehicle, eta):

    DOF = 6  # degrees of freedom
    t = 0  # initial simulation time

    # Initial state vectors
    eta = np.zeros(6, float)  # position/attitude, user editable [x,y,z,roll,pitch,yaw]
    ...
    for i in range(0, N + 1):
        ...
        # update velocity(nu)
        [nu, u_actual, tau] = vehicle.dynamics(eta, nu, u_actual, u_control, sampleTime)
        # update attitude(eta)
        eta = attitudeEuler(eta, nu, sampleTime)

                # Store simulation data in simData
        signals = np.append(np.append(eta, nu), tau)
        # signals = [x,y,z,roll,pitch,yaw,u,v,w,p,q,r,u_control(3,),u_actual(3,)]
